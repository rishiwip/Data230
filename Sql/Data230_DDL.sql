...
CREATE OR REPLACE VIEW HR.VW_OLYMPIC_DATA_HIST_AGGR
AS
SELECT O.YEAR
	,O.GAMES
	,O.SEASON
	...
	,COUNT(O.medal) AS MEDAL_TOTAL
	,COUNT(DECODE(O.MEDAL, 'Gold', O.MEDAL, NULL)) AS GOLD_MEDAL
	,COUNT(DECODE(O.MEDAL, 'Silver', O.MEDAL, NULL)) AS Silver_MEDAL
	,COUNT(DECODE(O.MEDAL, 'Bronze', O.MEDAL, NULL)) AS Bronze_MEDAL
FROM HR.VW_OLYMPIC_DATA_HIST O
LEFT JOIN HR.VW_COUNTRIES_GDP_HIST G ON O."YEAR" = G."YEAR"
	AND O.NOC = G.COUNTRY_CODE
GROUP BY O.YEAR
	,O.GAMES
	,O.SEASON
	,O.AGE
